        ...
        let alternatives = lines.reduce((result, inputType) => {
            if (inputType.type === 'String') {
                ...
                if (field.name.endsWith('Tr')) {
                    result.push(this.wrapWithZodValidators(`z.object({${Transformer.languages
                        .map((lang) => `${lang}: z.string()`)
                        .join(', ')}})`, field, inputType));
                }
                else {
    ...
    async generateModelSchemas() {
        for (const modelOperation of this.modelOperations) {
            ...
            if (createMany) {
                ...
                await (0, writeFileSafely_1.writeFileSafely)(path_1.default.join(Transformer.outputPath, `schemas/${createMany}.schema.ts`), `${this.generateImportStatements(imports)}${this.generateExportSchemaStatement(`${modelName}CreateMany`, `z.object({ data: ${modelName}CreateManyInputObjectSchema  })`)}`);
            }
            if (deleteOne) {
                const imports = [
                    selectImport,
